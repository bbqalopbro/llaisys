...
__global__ __launch_bounds__(Warps * 32) void decode(
    ...
    // Adjacent lanes own adjacent quantization groups. Each warp issues
    // coalesced 16-byte packed weight loads; each weight group is reused for
    // all M rows. A remains packed and is served from the read-only/L1 cache.
    for (int group = lane; group < groups; group += 32) {
        ...
        for (int row = 0; row < M; ++row) {
            ...
            // Keep the published W4A8 per-K32 scaling boundary. Explicit RN
            // operations prevent contraction across the scaling boundary.
