"""Explicit full-model correctness probe for the experimental B300 W4A8 kernel.

One independent 43-layer model loads the real MP1 checkpoint once. Only FP4
GEMM changes: flattened M=1..8 uses standalone CUDA; M>8 uses the registered
...
Defaults to four short prompts plus the 134-token original published golden.
Use --suite chat for eight published chat cases with checkpoint payload and
golden-file hash bindings, or --suite top512 for the 2105-token golden. The
legacy short/134-token reports lack those bindings; the report says so.
"""
...
BACKEND = "experimental-w4a8-sm103-small-m-tilelang-large-m"
...
class ShapeDispatch:
    ...
    def __init__(self, native, tilelang_gemm, variant):
        self.native, self.tilelang_gemm, self.variant = native, tilelang_gemm, variant
    ...
    def __call__(self, a, a_s, b, b_s, scale_dtype=None):
        ...
        if scale_dtype != torch.float8_e8m0fnu:
            raise ValueError("this explicit W4A8 model probe requires UE8M0 scales")
        m, n, k = _validate_gemm("fp4", a, a_s, b, b_s, scale_dtype)
        route = "empty" if m == 0 else "cuda_small_m" if m <= 8 else "tilelang_large_m"
        self.attempts[route] += 1
        self.shapes[(route, m, n, k)] += 1
        try:
            if route == "empty":
                result = torch.empty((*a.shape[:-1], n), device=a.device, dtype=torch.bfloat16)
            elif route == "cuda_small_m":
                result = self.native.gemm(a.reshape(m, k), a_s.reshape(m, k // 128),
                                          b, b_s, variant=self.variant)
    ...
    def report(self):
        return {
            "policy": "M=0: empty; M=1..8: standalone CUDA; M>8: registered TileLang",
            "policy_selected_before_execution": True, "error_fallback": False,
            "cuda_variant": self.variant,
...
def run(args, report):
    ...
    spec = importlib.util.spec_from_file_location("llaisys_w4a8_probe_reference", REFERENCE / "run_independent.py")
    ...
    dispatch = ShapeDispatch(native, baseline_ops.function("fp4_gemm"), args.variant)
    registry.register(BACKEND, "fp4_gemm", dispatch, version=native.version,
                      contract=CONTRACTS["fp4_gemm"])
    selection["fp4_gemm"] = BACKEND
    ...
    def live_evidence():
        report.update(operator_backends=ops.report(), w4a8_dispatch=dispatch.report(),
    ...
    try:
        for index, case in enumerate(cases):
            ...
            result["w4a8_calls"] = {key: count - counts_before.get(key, 0)
                                     for key, count in dispatch.completed.items()}
            report["cases"].append(result)
            live_evidence()
            write_report(args.output, report)
            print(f"case {index}: tokens={result['input_tokens']} steps={len(result['steps'])} "
                  f"numerical_gate={result['numerical_gate_passed']} dispatch={result['w4a8_calls']}", flush=True)
        torch.cuda.synchronize()
    ...
    report["both_shape_routes_exercised"] = all(dispatch.completed.get(k, 0) > 0
                                                for k in ("cuda_small_m", "tilelang_large_m"))
...
def main():
    ...
    parser.add_argument("--suite", choices=("short-long", "chat", "top512"), default="short-long")
    ...
    parser.add_argument("--variant", type=int, choices=(1, 2, 3), default=1,
                        help="one fixed native variant for every small-M call; no measured runtime selection")
    ...
    report = {
        ...
        "executor": "llaisys-independent-43-layer-explicit-w4a8-probe",
