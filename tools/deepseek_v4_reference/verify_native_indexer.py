"""Actual-weight native Indexer versus published full/decode and a chunk oracle.

Python prepares fixtures/TileLang libraries only. Native child executes the
complete Indexer without Python. This does not validate full Attention/model,
...
from llaisys.models.deepseek_v4_model.layers import Indexer as IncrementalIndexer
...
def main(args):
    if not os.environ.get("SLURM_JOB_ID") or not torch.cuda.is_available():
        raise RuntimeError("native Indexer verification requires Slurm GPU allocation")
    import tilelang
    import tvm_ffi
    ...
    if (config.dim, config.q_lora_rank, config.index_n_heads, config.index_head_dim, config.rope_head_dim,
        config.index_topk, config.compress_ratios[2], config.compress_ratios[42]) != (4096, 1024, 64, 128, 64, 512, 4, 4):
        raise RuntimeError("actual Flash-0731 Indexer configuration required")
    ...
    sources = [Path(__file__), root / "test/indexer_native_standalone.cpp", root / "test/native_fixture_utils.hpp",
    ...
    spec = importlib.util.spec_from_file_location("published_indexer_oracle", inference / "model.py")
    ...
    artifacts = Path(tempfile.mkdtemp(prefix="native_indexer_", dir=args.output.parent)).resolve()
    ...
    exports, origins = {}, {}
    def export(operation, compiled):
        path = artifacts / f"{operation}.so"; cached = getattr(compiled.adapter, "libpath", None)
        if cached:
        ...
        exports[operation] = path
    export("act_quant", kernel.act_quant_kernel(cfg.q_lora_rank, 128, scale_dtype=kernel.FE8M0, round_scale=True))
    export("fp8_gemm", kernel.fp8_gemm_kernel(cfg.index_n_heads * cfg.index_head_dim, cfg.q_lora_rank, scale_dtype=kernel.FE8M0))
    export("fp4_act_quant", kernel.fp4_quant_kernel(cfg.index_head_dim, 32, inplace=True))
    components = []
    hadamard_version = sha256(args.hadamard_source / "csrc/fast_hadamard_transform_cuda.cu")
    lines = [f"LLAISYS_NATIVE_INDEXER_V1 {tilelang.__version__} {torch.__version__.split('+')[0]} {hadamard_version} 2"]
    with torch.inference_mode(), safe_open(checkpoint, framework="pt", device="cpu") as reader:
        for layer in (2, 42):
            prefix = f"layers.{layer}.attn.indexer"
            official = published.Indexer(cfg)
            incremental = IncrementalIndexer(config, ops, hadamard.hadamard_transform)
            names = set(dict(official.named_parameters()))
            actual = {key.removeprefix(prefix + ".") for key in reader.keys() if key.startswith(prefix + ".")}
            if names != actual or names != set(dict(incremental.named_parameters())) or len(names) != 7:
                raise RuntimeError("strict Indexer checkpoint key mapping failed")
            records = []
            for name, param in official.named_parameters():
                value = reader.get_tensor(prefix + "." + name)
                converted = name.startswith("compressor.") and param.dtype == torch.float32 and value.dtype == torch.bfloat16
                if value.shape != param.shape or (value.dtype != param.dtype and not converted):
                    raise RuntimeError(f"actual Indexer weight shape/dtype mismatch: {name}")
            ...
            if official.wq_b.weight.scale is not official.wq_b.scale: raise RuntimeError("published weight.scale alias broken")
            freq_record = fixture("frequencies", frequencies, prefix)
            lines.append(" ".join(map(str, [prefix, cfg.dim, cfg.q_lora_rank, cfg.index_n_heads, cfg.index_head_dim,
                                          cfg.rope_head_dim, cfg.index_topk, cfg.max_seq_len, cfg.norm_eps, 8])))
            lines.extend(line(item) for item in [*records, freq_record]); lines.append(str(len(exports)))
            lines.extend(f"{op} {json.dumps(str(path))}" for op, path in exports.items())
            plans = [("published_short", [3, 1, 1], "published"), ("published_partial", [137, 1, 1], "published"),
                     ("published_top512", [2105, 1, 1], "published"),
            ...
            for name, sizes, oracle in plans:
                cache = torch.zeros(1, cfg.max_seq_len // 4, cfg.index_head_dim, dtype=torch.bfloat16, device="cuda")
                state = LayerState(None, frequencies, None, cache, CompressorState.allocate(1, 4, cfg.index_head_dim, "cuda"))
                official.kv_cache = cache; official.freqs_cis = frequencies
                official.compressor.kv_cache = cache; official.compressor.freqs_cis = frequencies
                ...
                traces = {}; hooks = []
                for tag, module in (("projected_query", model.wq_b), ("projected_weights", model.weights_proj)):
                ...
                try:
                    for index, tokens in enumerate(sizes):
                        ...
                        qr = torch.randn(1, tokens, cfg.q_lora_rank, dtype=torch.bfloat16, device="cuda")
                        offset = (0, 128, 257)[index % 3]
                        output = official(x, qr, position, offset) if oracle == "published" else incremental(x, qr, position, offset, state)
                        # Additional diagnostic derived from the observed query projection.
                        # The authoritative final indices/cache above come from unchanged forward.
                        q = traces["projected_query"].unflatten(-1, (cfg.index_n_heads, cfg.index_head_dim))
                        published.apply_rotary_emb(q[..., -cfg.rope_head_dim:], frequencies[position:position + tokens])
                        q = published.rotate_activation(q); kernel.fp4_act_quant(q, 32, True)
                        expected = {"input": x, "query_rank": qr, "output": output.int(), "cache": cache,
                            "kv_state": official.compressor.kv_state if oracle == "published" else state.index_compressor.kv,
                            "score_state": official.compressor.score_state if oracle == "published" else state.index_compressor.scores,
                            "projected_weights": traces["projected_weights"], "quantized_query": q}
                        specs = [fixture(tag, value, f"{prefix}_{name}_{index}") for tag, value in expected.items()]
                        lines.append(f"{position} {tokens} {offset} {len(specs)}"); lines.extend(line(item) for item in specs)
                        steps.append({"position": position, "tokens": tokens, "offset": offset, "tensors": specs}); position += tokens
                finally:
                    for hook in hooks: hook.remove()
                results.append({"name": name, "oracle": oracle, "tokens": sizes, "steps": steps})
                print(f"reference {prefix}/{name}: {sizes}", flush=True)
            components.append({"name": prefix, "weights": records, "frequencies": freq_record, "plans": results})
    manifest = artifacts / "manifest.txt"; manifest.write_text("\n".join(lines) + "\n")
    executable = artifacts / "indexer-native-standalone"
    ...
    command = [shutil.which("g++"), "-std=c++20", "-O2", "-pthread", "-D_GLIBCXX_USE_CXX11_ABI=1",
        *["-I" + str(path) for path in (root, root / "include", ffi_root / "include", torch_root / "include",
            torch_root / "include/torch/csrc/api/include", Path(CUDA_HOME) / "include")], str(root / "test/indexer_native_standalone.cpp"),
    ...
    report = {"scope": "actual-weight native Indexer; NOT full Attention/model, paged cache or performance", "result": result,
        ...
        "configuration": {"hidden": cfg.dim, "query_rank": cfg.q_lora_rank, "heads": cfg.index_n_heads,
                          "head_dim": cfg.index_head_dim, "rope_dimension": cfg.rope_head_dim, "capacity": cfg.max_seq_len,
                          "topk": cfg.index_topk, "compress_ratio": 4, "batch": 1, "norm_epsilon": cfg.norm_eps,
                          "frequency_preparation": "published precompute_freqs_cis outside native execution",
                          "precision": "W8A8 query GEMM, BF16 query/cache/scores/head projection, FP32 compressor, FP4 QDQ block32",
                          "tie_policy": "published topk, not index_ascending", "inputs": "deterministic random BF16 hidden/query, not text"},
        "reference_scope": {"published": "unchanged upstream Indexer.forward: initial prefill and single-token decode",
                            "incremental_python": "existing project Indexer: SAME multi-token chunk schedule, not original full-prefill oracle",
                            "quantized_query": "diagnostic from captured projection plus unchanged upstream RoPE/Hadamard/quant"},
...
if __name__ == "__main__":
    ...
    try: main(args)
        ...
        args.output.parent.mkdir(parents=True, exist_ok=True)
        args.output.write_text(json.dumps({"all_passed": False, "scope": "native Indexer verification incomplete",
